...
const InitProgress = memo<InitingProps>(({ activeStage, stages }) => {
  ...
  const safeActiveStage = Math.max(0, Math.min(activeStage, stages.length - 1));
  const outStage = stages[safeActiveStage] || stages[0] || 'Loading...';
  const percent = stages.length > 1 ? (safeActiveStage / (stages.length - 1)) * 100 : 0;
