...
const VirtualizedList = memo<VirtualizedListProps>(({ mobile, dataSource, itemContent }) => {
  ...
  useEffect(() => {
    if (isClient && virtuosoRef.current) {
  ...
  const overscan = isClient && typeof window !== 'undefined' ? window.innerHeight * 3 : 0;

  // first time loading or not loaded
  if (isFirstLoading || !isClient) return <SkeletonList mobile={mobile} />;
